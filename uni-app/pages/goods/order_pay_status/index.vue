		...
		<view class="payment-status" v-if="(!orderLottery || !order_pay_info.paid || is_gift) && loading && lotteryLoading">
			...
			<view class="wrapper">
				...
				<view class="item acea-row row-between-wrapper">
					...
					<view class="item acea-row row-between-wrapper">
						<view>{{ $t(`支付方式`) }}</view>
						<view class="itemCom">{{ displayPayType }}</view>
					</view>
				<view class="item acea-row row-between-wrapper">
					<view>{{ $t(`支付金额`) }}</view>
...
export default {
	...
	computed: {
		...mapGetters(['isLogin']),
		displayPayType() {
			if (this.payType === 'allinpay') return this.$t(`银联支付`);
			return this.$t(this.order_pay_info._status._payType) || this.$t(`暂未支付`);
		}
	},
	watch: {
		isLogin: {
	...
	onLoad(options) {
		...
		this.orderId = options.order_id;
			...
			this.forcePaid = Number(options.paid || 0) === 1;
		...
		getOrderPayInfo: function () {
			...
			uni.showLoading({
				...
				getOrderDetail(that.orderId)
					.then((res) => {
						uni.hideLoading();
						if (that.forcePaid && that.payType === 'allinpay') {
							res.data.paid = 1;
							res.data.pay_type = 'allinpay';
							res.data._status = res.data._status || {};
							res.data._status._payType = '银联支付';
						}
