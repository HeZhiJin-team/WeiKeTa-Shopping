<template>
	<view class="wrapper" :style="colorStyle">
		...
		<view class="page-msg">
			<view class="title">
				{{pageType == 1?$t('绑定邮箱'):$t('邮箱登录')}}
			</view>
			<view class="tip">
				{{pageType == 1?$t('登录注册需绑定邮箱'):$t('首次登录会自动注册')}}
			</view>
		</view>
		<view class="page-form">
			<view class="item">
				<input type='text' :placeholder='$t(`填写邮箱`)' placeholder-class='placeholder' v-model="phone"></input>
			</view>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.pageType) {
				...
				this.pageTitle = options.pageType == 1 ? '绑定邮箱' : '邮箱登录'
			}
		...
		methods: {
			...
			rules() {
				...
				if (!checkEmail(that.phone)) {
					that.$util.Tips({
						title: that.$t(`请输入正确的邮箱`)
					});
					return false
				}
				if (!that.captcha) {
			...
			async code() {
				...
				if (!checkEmail(that.phone)) return that.$util.Tips({
					title: that.$t(`请输入正确的邮箱`)
				});
